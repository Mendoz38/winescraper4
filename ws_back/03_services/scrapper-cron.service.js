const { getAllScrappers, runScrapper } = require('../02_models/scrapper.model');
...
function shouldRunNow({ dayCron, hourCron, now }) {
  ...
  if (parsedHour.hour !== now.getHours() || parsedHour.minute !== now.getMinutes()) return false;
  if (!dayCron) return true;
  const expectedDay = normalizeDay(dayCron);
  if (!expectedDay) return false;
  return DAY_ORDER[now.getDay()] === expectedDay;
}
...
function createCronScheduler() {
  ...
  async function runScheduledScrapes() {
    ...
    try {
      const scrappers = await getAllScrappers({ activeOnly: true });
      for (const scrapper of scrappers) {
        if (!shouldRunNow({ dayCron: scrapper.day_cron, hourCron: scrapper.hour_cron, now })) continue;
  ...
  function start() {
    ...
    console.log(`[cron] scheduler started interval=${CRON_INTERVAL_MS}ms`);
    return setInterval(() => {
      runScheduledScrapes().catch(() => {});
    }, CRON_INTERVAL_MS);
  }

  return { start, runScheduledScrapes };
}
